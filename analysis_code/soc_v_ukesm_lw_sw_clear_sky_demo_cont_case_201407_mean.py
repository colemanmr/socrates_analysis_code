...
file_ukesm_data = file_loc.diag_dir + '/socrates_diags/cd964_201407_toa_fluxes.pp' # need to redo this with 30 day means and mean them sadly as didn't add a monthly mean diag for all flux types to suite :/

file_soc_toa_lw = file_loc.diag_dir + '/socrates_diags/bc_prp_1year_clear_sky_cont_201407_toa_lw.nflx'
file_soc_toa_sw = file_loc.diag_dir + '/socrates_diags/bc_prp_1year_clear_sky_cont_201407_toa_sw.nflx'
...
# difference plot
plt.figure()
plt.title('TOA net down flux 201407 Socrates - UKESM prp bc demo cont case clear')
mesh = iplt.pcolormesh(net_diff, vmin = -1, vmax = 1, cmap = 'seismic')
plt.colorbar(mesh, shrink = 0.9, label = u'Flux / W m$^{-2}$', \
                  orientation = 'horizontal',
                   #ticks = [-0.02, -0.01, 0, 0.01, 0.02]
                  )
plt.savefig(plot_dir + 'net_flux_toa_map_201407_soc-ukesm_prp_bc_demo_cont_case_clear.png', dpi = 300)
plt.show()

# sw difference plot
plt.figure()
plt.title('TOA sw net down flux 201407 Socrates - UKESM prp bc demo cont case clear')
mesh = iplt.pcolormesh(net_diff_sw, vmin = -1, vmax = 1, cmap = 'seismic')
plt.colorbar(mesh, shrink = 0.9, label = u'Flux / W m$^{-2}$', \
                  orientation = 'horizontal',
                   #ticks = [-0.02, -0.01, 0, 0.01, 0.02]
                  )
plt.savefig(plot_dir + 'net_sw_flux_toa_map_201407_soc-ukesm_prp_bc_demo_cont_case_clear.png', dpi = 300)
plt.show()

# lw difference plot
plt.figure()
plt.title('TOA lw net down flux 201407 Socrates - UKESM prp bc demo cont case clear')
mesh = iplt.pcolormesh(net_diff_lw, vmin = -1, vmax = 1, cmap = 'seismic')
plt.colorbar(mesh, shrink = 0.9, label = u'Flux / W m$^{-2}$', \
                  orientation = 'horizontal',
                   #ticks = [-0.02, -0.01, 0, 0.01, 0.02]
                  )
plt.savefig(plot_dir + 'net_lw_flux_toa_map_201407_soc-ukesm_prp_bc_demo_cont_case_clear.png', dpi = 300)
plt.show()
